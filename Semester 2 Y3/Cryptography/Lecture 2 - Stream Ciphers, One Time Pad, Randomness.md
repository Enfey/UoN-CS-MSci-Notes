...
- These are not suitable for cryptography; given enough output, the internal state can be recovered. Future bits become predictable given that they are formed according to prior bits. 
	- If they recover the internal state $s_i$ and $a, c$ are known (if not hardening, will have selected standard values from the literature), all future output can be calculated, breaking the keystream.
...
- Key stream $s_0, s_1, s_2$ generated by a **CSPRNG.**
